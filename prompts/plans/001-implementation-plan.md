...
- 컨테이너 예제와 `.env.example`는 `WORKER_ISSUE_KEY_PATH`, `WORKER_PROCESS_PATH`, `WORKER_TIMEOUT_MS`를 노출해 Worker 연동 설정 override를 image rebuild 없이 바꿀 수 있어야 한다.
- compose 기본값은 `application.yml`의 Worker 기본 경로와 같아야 하고, `docker compose -f docker/compose.yaml config` 검증을 통과해야 한다.
